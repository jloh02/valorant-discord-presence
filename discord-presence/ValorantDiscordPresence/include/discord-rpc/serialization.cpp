...

	{
...

		{
			...
			if (presence != nullptr) {
				...
				if (presence->buttonAlabel && presence->buttonAlabel[0] && presence->buttonAurl && presence->buttonAurl[0]) {
					WriteArray buttons(writer, "buttons");
					WriteObject buttonA(writer);
					WriteOptionalString(writer, "label", presence->buttonAlabel);
					WriteOptionalString(writer, "url", presence->buttonAurl);
					if (presence->buttonBlabel && presence->buttonBlabel[0] && presence->buttonBurl && presence->buttonBurl[0]) {
						WriteObject buttonB(writer);
						WriteOptionalString(writer, "label", presence->buttonBlabel);
						WriteOptionalString(writer, "url", presence->buttonBurl);
					}
				}
			}
		}
